...
def main():
    base_dir = 'C:\\Users\\tzbz_\\Documents\\Pytorch\\FCN-DenseNet' #/home/zeng/data/datasets/saliency_Dataset
    algs = ['duts_res50', 'duts_res101'] #algs = ['duts_res50', 'duts_res101']
    datasets = ['ECSSD'] #datasets = ['ECSSD', 'SOD']
...
def evaluate(input_dir, gt_dir, output_dir=None, name=None):
    ...
    for filename in filelist:
        ...
        recs = []
        fms = []
        mea = np.abs(gt-mask).mean() #gt-mask就是求出不为0的像素点个数，因为gt读到的图片就是黑色和目标图像的特殊颜色，#平均绝对误差mae#
        # threshold fm
